  ...
  if ((uo$COD_ORGAO[1] %in% registro_orgao) == FALSE) { 
    cat("\\addcontentsline{toc}{section}{", paste(substr(uo$COD_ORGAO[1], 1, 1), ".", substr(uo$COD_ORGAO[1], 2, 3), ".", 
      substr(uo$COD_ORGAO[1], 4, 4), " - ", uo$ORGAO[1], 
      sep = "" 
    ), "}\n") 
  } 
  ...
  cat("\\addcontentsline{toc}{subsection}{", paste(substr(uo$COD_UO[1], 1, 1), ".", substr(uo$COD_UO[1], 2, 3), ".", 
    substr(uo$COD_UO[1], 4, 4), " - ", uo$UO[1], 
    sep = "" 
  ), "}\n") 
  ...
  titulo <- "\\multicolumn{16}{c}{\\cellcolor{gray!50} \\textcolor{myblack} {\\footnotesize \\textbf{QUADRO DE DETALHAMENTO DA DESPESA - FISCAL}}}\\TBstrut  \\\\[2ex]\n" 
 
  subtitulo <- paste( 
    "\\multicolumn{15}{l}{\\textbf{Exercício:} \\textcolor{myblack}{", uo$ANO[1], 
    "}} & \\multicolumn{1}{r}{ R\\$1,00} \\\\\n" 
  ) 
 
  titulo_orgao <- paste("\\multicolumn{16}{L{15cm}}{\\textbf{ÓRGÃO:}", paste(substr(uo$COD_ORGAO[1], 1, 1), ".", 
  ...
  titulo_uo <- paste("\\multicolumn{16}{L{15cm}}{\\textbf{UO:} \\hspace{13pt} ", paste(substr(uo$COD_UO[1], 1, 1), ".", 
  ...
  cabecalho1 <- paste( 
    "\\multirow{2}{3pt}{ESPECIFICAÇÃO} & \\multicolumn{13}{|c|}{CLASSIFICAÇÃO ORÇAMENTÁRIA} &", 
    "\\multicolumn{2}{c}{VALOR} \\\\\n" 
  ) 
 
  cabecalho2 <- paste( 
    "& \\multicolumn{1}{|c}{\\fonteSeis FUN} & \\multicolumn{1}{c}{\\fonteSeis SUBF} & ", 
    ...
    "\\multicolumn{1}{c|}{\\fonteSeis IPU} & \\multicolumn{1}{c|}{\\fonteSeis DETALHE} & ", 
  ...
  for (i in 1:nrow(uo)) { 
    if (toupper(uo$NOME_ACAO[i]) != "TOTAL" & uo$NOME_ACAO[i] != "") { 
      cat( 
        "\\multicolumn{1}{L{5.5cm}|}{", uo$NOME_ACAO[i], 
    ...
    if (toupper(uo$NOME_ACAO[i]) == "TOTAL") { 
      cat("\\hline\n") 
      cat( 
        "\\multicolumn{15}{C{6cm}|}{\\textbf{", toupper(uo$NOME_ACAO[i]), 
