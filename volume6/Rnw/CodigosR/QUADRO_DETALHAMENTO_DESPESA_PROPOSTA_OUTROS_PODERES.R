...
subtitulo <- paste(
  "\\multicolumn{17}{l}{\\textbf{Exercício:} \\textcolor{myblack}{", uo$ANO[1],
...
for (i in 1:nrow(uo)) {
  if (toupper(uo$NOME_ACAO[i]) != "TOTAL" & uo$NOME_ACAO[i] != "") {
    cat(
      "\\multicolumn{1}{L{4.8cm}|}{", uo$NOME_ACAO[i],
...
cat(
  "\\fontsize{7pt}{10pt}\\selectfont {(1) Proposta orçamentária encaminhada pelo",
  gsub("(.+) *- * (.+)", "\\2", uo$UO[1]), "}\\\\\n"
)
