...
if("Apenas no Banco X" %in% consolidado[, unique(merge)]){
  warning(paste("volume5.R banco consolidado: Há códigos de despesa em BASE_QDD_FISCAL.xlsx que não possuem",
...
write.table(consolidado, "volume5/data/consolidado.txt", quote = FALSE, sep = "\t", 
            na = "", dec = ",", row.names = FALSE)


# Inicio do QUADRO DE DETALHAMENTO DA DESPESA - FISCAL

banco[, cod_detalhe := paste0(formatC(FUNCAO, width = 2, flag = "0"), 
                              formatC(SUB_FUNCAO, width = 3, flag = "0"),
                              PROGRAMA, 
                              IDENT_PROJATIV, 
                              formatC(PROJ_ATIV, width = 3, flag = "0"), 
