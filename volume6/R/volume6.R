...
modalidade0 <- consolidado[, list(
  outras_fontes = sum(outras_fontes, na.rm = T),
...
modalidade0 <- rbind(modalidade0, data.table(
  codigo_numerico = 30,
...
modalidade0[, codigo_texto := paste0(
  substr(codigo_numerico, 1, 1), ".", substr(codigo_numerico, 2, 2), ".",
...
if ("Apenas no Banco X" %in% consolidado[, unique(merge)]) {
  warning(paste(
    "volume5.R banco consolidado: Há códigos de despesa em BASE_QDD_FISCAL.xlsx que não possuem",
...
write.table(consolidado, "volume5/data/consolidado.txt",
  quote = FALSE, sep = "\t",
...
write.table(banco, "volume5/data/QUADRO_DETALHAMENTO_DESPESA_porUO.txt",
