...
# Captura total da receita fiscal (numérico) antes de formatar
total_rec_fiscal = painel_rec[ordem_rec == "10-02-00-0",
                              list(ord_rec  = ordinaria_rec,
                                   vinc_rec = vinculada_rec,
                                   tot_rec  = total_rec)]
...
# Captura total da despesa fiscal (numérico) antes de formatar
total_desp_fiscal = painel_desp[ordem == "9-3-0-0-0",
                                list(ord_desp  = ordinaria,
                                     vinc_desp = vinculada,
                                     tot_desp  = total)]
...
# =========================================================================
# Monta o demonstrativo receita + despesa
...
# Linha "RESULTADO FISCAL"  =  Total Receita Fiscal - Total Despesa Fiscal
res_fiscal = data.table(
  receita        = "RESULTADO FISCAL",
  ordinaria_rec  = formatarNum(total_rec_fiscal$ord_rec  - total_desp_fiscal$ord_desp),
  part_ord_rec   = "",
  vinculada_rec  = formatarNum(total_rec_fiscal$vinc_rec - total_desp_fiscal$vinc_desp),
  part_vinc_rec  = "",
  total_rec      = formatarNum(total_rec_fiscal$tot_rec  - total_desp_fiscal$tot_desp),
  part_total_rec = "",
  despesa        = "",
  ordinaria      = "",
  part_ord       = "",
...
demonstr = rbind(demonstr, res_fiscal, fill = T)

setnames(demonstr,
         old = c("receita",  "ordinaria_rec", "part_ord_rec", "vinculada_rec",
                 "part_vinc_rec", "total_rec", "part_total_rec",
                 "despesa",  "ordinaria",   "part_ord",    "vinculada",
                 "part_vinc",   "total",     "part_total"),
         new = c("RECEITA ORÇAMENTÁRIA", "ORDINÁRIA", "% PART", "VINCULADA",
                 "% PART", "TOTAL", "% PART",
                 "DESPESA ORÇAMENTÁRIA", "ORDINÁRIA", "% PART", "VINCULADA",
                 "% PART", "TOTAL", "% PART"))

write.table(demonstr, "volume1/data/T1_DEMONSTRATIVO_CONSOLIDADO_ORCAMENTO_FISCAL.txt",
