demonstr_mde_rec = function(loa_rec){
  ...
  base[is_ibs_bruto(base),
       c("nvl2", "lvl2"):= list("1.4 - IBS", 140)]

  base[is_ibs_principal(base),
       c("nvl3", "lvl3"):= list("1.4.1 - IBS Principal", 141)]

  base[is_ibs_acessorias(base),
       c("nvl3", "lvl3"):= list("1.4.2 - Multas, Juros de Mora, Dívida Ativa e Outros Encargos do IBS",
                                142)]

  # IRRF

  base[is_irrf_bruto(base),
       c("nvl2", "lvl2"):= list("1.5 - IRRF",
                                150)]
  ...
  base[is_ibs_bruto(base) & substr(RECEITA_COD, 10, 10) == "2",
       c("nvl2d", "lvl2d"):= list("3.4 - Parcela do IBS Repassada aos Municípios", 340)]

  base[is_itcd_bruto(base) & FONTE_COD == 20,
       c("nvl2d", "lvl2d"):= list("3.5 - Parcela do ITCD Repassada aos Municípios", 350)]

  base[nat(RECEITA_COD, 17196201) & FONTE_COD == 20,
       c("nvl2d", "lvl2d"):= list("3.6 - Parcela da LC nº 194/2022 Repassada aos Municípios", 360)]
