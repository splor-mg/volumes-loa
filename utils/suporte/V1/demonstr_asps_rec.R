demonstr_asps_rec = function(loa_rec){
  ...
  base[is_ibs_principal(base), c("nvl2", "lvl2"):= list("IBS Principal", 140)]

  base[is_irrf_bruto(base), c("nvl2", "lvl2"):= list("IRRF", 150)]

  base[is_icms_mjm(base) | is_itcd_mjm(base) | is_ipva_mjm(base),
       c("nvl2", "lvl2"):= list("Multas, Juros de Mora e Outros Encargos dos Impostos", 160)]

  base[is_icms_divida_ativa(base)  | is_itcd_divida_ativa(base) | is_ipva_divida_ativa(base),
       c("nvl2", "lvl2"):= list("Dívida Ativa dos Impostos", 170)]


  base[is_icms_mjm_divida_ativa(base) | is_ipva_mjm_divida_ativa(base),
       c("nvl2", "lvl2"):= list("Multas, Juros de Mora e Outro Encargos da Dívida Ativa", 180)]


  base[is_irrf_bruto(base) | is_ipva_bruto(base) | is_itcd_bruto(base) | is_icms_bruto(base) | is_ibs_bruto(base),
  ...
  base[is_ibs_bruto(base) & substr(RECEITA_COD, 10, 10) == "2",
     c("nvl2d", "lvl2d"):= list("Parcela do IBS Repassada aos Municípios", 340)]

  base[is_itcd_bruto(base) & FONTE_COD == 20,
       c("nvl2d", "lvl2d"):= list("Parcela do ITCD Repassada aos Municípios", 350)]

  base[nat(RECEITA_COD, 17196201) & FONTE_COD == 20,
       c("nvl2d", "lvl2d"):= list("Parcela da LC nº 194/2022 Repassada aos Municípios", 360)]
