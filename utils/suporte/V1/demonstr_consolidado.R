...
demonstrativo_consolidado_rec = function(loa_rec){
  ...
  base[nat(RECEITA_COD, 9) & is_transf_const_mun_rec(base) & is_icms_bruto(base, deducoes = T), c("nvl3", "desc3") :=list(30,"ICMS - MUNICÍPIOS")]

  base[nat(RECEITA_COD, 9) & is_ida_fundeb(base) & is_ipva_bruto(base, deducoes = T), c("nvl3", "desc3") :=list(25,"IPVA - FUNDEB")]
  base[nat(RECEITA_COD, 9) & is_transf_const_mun_rec(base) & is_ipva_bruto(base, deducoes = T), c("nvl3", "desc3") :=list(31,"IPVA - MUNICÍPIOS")]

  base[nat(RECEITA_COD, 9) & is_ida_fundeb(base) & is_itcd_bruto(base, deducoes = T), c("nvl3", "desc3") :=list(26,"ITCD - FUNDEB")]
  base[nat(RECEITA_COD, 9) & is_transf_const_mun_rec(base) & is_itcd_bruto(base, deducoes = T), c("nvl3", "desc3") :=list(32,"ITCD - MUNICÍPIOS")]

  base[nat(RECEITA_COD, 9) & is_ida_fundeb(base) & is_ibs_bruto(base, deducoes = T), c("nvl3", "desc3") := list(27, "IBS - FUNDEB")]
  base[nat(RECEITA_COD, 9) & is_transf_const_mun_rec(base) & is_ibs_bruto(base, deducoes = T), c("nvl3", "desc3") := list(35, "IBS - MUNICÍPIOS")]
  base[nat(RECEITA_COD, 9) & substr(RECEITA_COD, 10, 10) == "4" & is_ibs_bruto(base, deducoes = T), c("nvl3", "desc3") := list(36, "IBS - COMITÊ GESTOR")]

  base[nat(RECEITA_COD, 9) & is_ida_fundeb(base) & is_fpe_principal(base, deducoes = T), c("nvl3", "desc3") :=list(28,"FPE - FUNDEB")]
  #base[nat(RECEITA_COD, 9) & is_transf_const_mun_rec(base) & is_fpe_principal(base), c("nvl3", "desc3") :=list(24,"FPE - MUNICÍPIOS")]

  base[nat(RECEITA_COD, 9) & is_ida_fundeb(base) & is_ipi_principal(base, deducoes = T), c("nvl3", "desc3") :=list(29,"IPI - FUNDEB")]
  base[nat(RECEITA_COD, 9) & is_transf_const_mun_rec(base) & is_ipi_principal(base, deducoes = T), c("nvl3", "desc3") :=list(33,"IPI - MUNICÍPIOS")]

  base[nat(RECEITA_COD, 971154), c("nvl3", "desc3") :=list(34,"CIDE - MUNICÍPIOS")]

  #base[nat(RECEITA_COD, 9) & is.na(desc3), c("nvl3", "desc3") :=list(34,"CESSÃO DE DIREITOS CREDITÓRIOS")]
  base[nat(RECEITA_COD, 9911010103003, 9911010103004) & is.na(desc3), c("nvl3", "desc3") :=list(37,"FUNSET")]
  base[nat(RECEITA_COD, 9) & is.na(desc3) , c("nvl3", "desc3") :=list(38,"OUTRAS DEDUÇÕES")]
  ...
  base[is_ibs_principal(base), c("nvl4", "desc4") := list(4, "IBS")]
  base[is_irrf_principal(base), c("nvl4", "desc4") :=list(5, "IRRF")]
  base[is_taxas_principal(base), c("nvl4", "desc4") :=list(6, "TAXAS")]
  ...
  base[is_icms_acessorias(base), c("nvl4", "desc4") :=list(7, "ICMS")]
  base[is_ipva_acessorias(base), c("nvl4", "desc4") :=list(8, "IPVA")]
  base[is_itcd_acessorias(base), c("nvl4", "desc4") :=list(9, "ITCD")]
  base[is_ibs_acessorias(base), c("nvl4", "desc4") := list(10, "IBS")]

  base[is_taxas_acessorias(base), c("nvl4", "desc4") :=list(11, "TAXAS")]
