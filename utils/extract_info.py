...
def get_versions_from_env():
    ...
    env_keys = ['RELATORIOS_VERSION', 'EXECUCAO_VERSION', 'REEST_VERSION', 'ANO_LOA_IMAGEM']
    env_values = {}
    for key in env_keys:
...
def update_config_mk(versions, config_path="config.mk"):
    ...
    for idx, line in enumerate(lines):
        if '# CONFIGURAÇÕES FLUIDAS' in line:
            start_idx = idx - 1  # Incluir a linha anterior com ===
            continue
    ...
    keys = ['ANO_LOA_IMAGEM', 'RELATORIOS_VERSION', 'EXECUCAO_VERSION', 'REEST_VERSION']
    ...
    ensure_kv('ANO_LOA_IMAGEM', versions.get('ANO_LOA_IMAGEM', 'unknown'), '# Versões extraídas')
...
def main():
    ...
    # TENTATIVA 1: Extrair labels da imagem Docker (host)
    print(f"\n{Colors.BLUE}Tentativa 1: Extraindo labels da imagem Docker (host){Colors.END}")
    labels = get_docker_image_info(docker_user, docker_image, docker_tag)
    if labels:
        versions = extract_versions(labels)
        source = "docker inspect (host)"
        print(f"{Colors.GREEN}✓ Sucesso: Labels extraídos da imagem Docker{Colors.END}")
    else:
        print(f"{Colors.YELLOW}⚠️  Falha: Docker indisponível ou execução dentro do container{Colors.END}")
        
        # TENTATIVA 2: Extrair labels da imagem atual (container)
        print(f"\n{Colors.BLUE}Tentativa 2: Extraindo labels da imagem atual (container){Colors.END}")
        
        # Tenta extrair labels da imagem atual usando diferentes métodos
        container_labels = None
        
        # Método 2a: Tentar docker inspect com imagem local
        try:
            if shutil.which('docker') is not None:
                # Tenta com a imagem local
                local_image = f"{docker_image}:{docker_tag}"
                cmd = ['docker', 'inspect', local_image, '--format={{json .Config.Labels}}']
                result = subprocess.run(cmd, capture_output=True, text=True, check=True)
                container_labels = json.loads(result.stdout)
                print(f"{Colors.GREEN}✓ Sucesso: Labels extraídos da imagem local{Colors.END}")
        except:
            pass
        
        # Método 2b: Tentar com imagem completa
        if not container_labels:
            try:
                if shutil.which('docker') is not None:
                    full_image = f"{docker_user}/{docker_image}:{docker_tag}"
                    cmd = ['docker', 'inspect', full_image, '--format={{json .Config.Labels}}']
                    result = subprocess.run(cmd, capture_output=True, text=True, check=True)
                    container_labels = json.loads(result.stdout)
                    print(f"{Colors.GREEN}✓ Sucesso: Labels extraídos da imagem completa{Colors.END}")
            except:
                pass
        
        # Método 2c: Tentar variáveis de ambiente como último recurso
        if not container_labels:
            env_vals = get_versions_from_env()
            if env_vals:
                container_labels = {
                    'relatorios.version': env_vals.get('RELATORIOS_VERSION', ''),
                    'execucao.version': env_vals.get('EXECUCAO_VERSION', ''),
                    'reest.version': env_vals.get('REEST_VERSION', ''),
                    'ano.loa': env_vals.get('ANO_LOA_IMAGEM', ''),
                }
                print(f"{Colors.GREEN}✓ Sucesso: Variáveis extraídas do ambiente do container{Colors.END}")
        
        if container_labels:
            versions = extract_versions(container_labels)
            source = "imagem atual (container)"
        else:
            print(f"{Colors.YELLOW}⚠️  Falha: Não foi possível extrair labels da imagem atual{Colors.END}")
            
            # TENTATIVA 3: Usar valores do config.mk (fallback final)
            print(f"\n{Colors.BLUE}Tentativa 3: Usando valores do config.mk (fallback final){Colors.END}")
            versions = {
                'ANO_LOA_IMAGEM': current_values.get('ANO_LOA_IMAGEM', 'unknown'),
            ...
            print(f"{Colors.YELLOW}⚠️  Fallback: Usando valores já definidos no config.mk{Colors.END}")
