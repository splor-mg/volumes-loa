...
<<results=tex,echo=FALSE>>=
  ...
  if(T %in% is.na(sumario$COD_UO)){
    warning("Há NA no banco de sumário. Ou seja, o(s) poder(es) ",
            paste0(sumario[is.na(COD_UO), poder], collapse=", "), " está sem Uo's")
  }

  for(aba in sort(na.omit(sumario$COD_UO))){
    #print(aba)
    banco = data.table(read.table(paste(dir_data, "/tabela1/",aba,".txt",sep=""),
                                  header=T, sep="\t", dec=",", stringsAsFactors = FALSE, quote = "\"'"))

    # banco = readr::read_delim(
    #   file = paste(dir_data, "/tabela1/",aba,".txt",sep=""),
    #   delim = '\t',
