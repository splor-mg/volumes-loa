    ...
    for(i in 1:nrow(banco)){ 
      ...
      cat("\\multicolumn{1}{c}{{\\bf ", formatC(TratamentoNA(banco[i, FUNCAO]), width = 2, flag = "0"), 
        ...
        cat("\\multicolumn{9}{L{8cm}|}{\\fontsize{6pt}{8pt}\\selectfont \\textit{", banco[i, descricao], 
          ...
          cat("\\multicolumn{6}{L{3cm}}{ \\fontsize{6pt}{8pt}\\selectfont ", 
              gsub("/", "/\\allowbreak{}", banco[i, descricao], fixed = TRUE), 
              "} & \\multicolumn{2}{C{2cm}}{\\fontsize{6pt}{8pt}\\selectfont ", 
              gsub("/", "/\\allowbreak{}", banco[i, unid_med_prod], fixed = TRUE), 
